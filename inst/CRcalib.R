...
# Simulate Data
sim.data <- riskRegression::sampleData(1000, outcome = 'competing.risks')

# Fit Fine-Gray Model
fgr.mod <- riskRegression::FGR(formula = Hist(time, event) ~ X7 + X6 + X8 + X9,
               data = sim.data,
               cause = 1)

# Predict cumulative incidence at median follow-up time
sim.data$pred <- riskRegression::predictRisk(fgr.mod, sim.data,
                                             times = median(sim.data$time))
